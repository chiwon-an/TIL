      ...
      <div class="results">
        ...
        {% if results %}
          <table class="ai-table">
            ...
            <tr>
              <td>{{loop.index}}</td>
              <td><img src="{{ai['image_url']}}" alt="logo"></td>
              <td>{{ai['name']}}</td>
              <td>{{ai['provider']}}</td>
              <td>{{ai['score']}}</td>
              <td>{{ai['pros']}}</td>
...
def index():
    ...
    if request.method == 'POST':
        ...
        filtered = []
        for ai in services:
            if form["budget"] != "상관없음" and form["budget"] not in ai["price"]:
                continue
            if form["language"] not in ai["language_support"]:
                continue
            if form["api"] and ai["api_support"] != "Yes":
                continue
            score = float(ai.get(purpose_key, 0))
            filtered.append({**ai, "score": score})
        filtered.sort(key=lambda x: x["score"], reverse=True)
        results = filtered[:3]
        # 후기 모으기 (추천된 AI의 reviews 필드에서 랜덤 1~2개씩)
        import random
        for ai in results:
            if ai.get("reviews"):
                review_list = ai["reviews"].split("|")
